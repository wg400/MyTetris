...
public class ModelN4 extends Model  {
	// LÐÍ
	public ModelN4(TetrisModel[][] tetrisModels) {
		super(tetrisModels);
	}
	...
	public void moveDown() {
		if (this.i<18) {
			this.i++ ;
			if (tetrisModels[i+1][j-1].isLock || tetrisModels[i+1][j].isLock || tetrisModels[i][j+1].isLock) {
				this.i-- ;
				lock() ;
			} else {
				this.i-- ;
