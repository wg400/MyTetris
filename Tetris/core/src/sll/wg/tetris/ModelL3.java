...
public class ModelL3 extends Model  {
	// LÐÍ
	public ModelL3(TetrisModel[][] tetrisModels) {
		super(tetrisModels);
	}
	...
	public void moveleft() {
		if (this.j>1) {
			this.j-- ;
			if (tetrisModels[i-1][j].isLock || tetrisModels[i+1][j-1].isLock || tetrisModels[i][j].isLock) {
				this.j++ ;
			} else {
				this.j++ ;
