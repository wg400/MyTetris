...
public class ModelT4 extends Model  {
	// LÐÍ
	public ModelT4(TetrisModel[][] tetrisModels) {
		super(tetrisModels);
	}
	...
	public void moveleft() {
		if (this.j>1) {
			this.j-- ;
			if (tetrisModels[i-1][j].isLock || tetrisModels[i][j-1].isLock || tetrisModels[i+1][j].isLock) {
				this.j++ ;
			} else {
				this.j++ ;
